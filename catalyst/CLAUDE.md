...
## Working Rules — AG Grid Recs Table

> Agreed rules from design sessions. Apply to every Claude Code session on `ReconciliationsPageAG.tsx`.

### Style Source of Truth
- **AG Grid Quartz theme** (`themeQuartz.withParams()`) is the style reference for everything inside the grid — spacing, color, typography, header treatment, cell rendering
- **FlowUI is NOT the style reference for grid elements** — do not map FlowUI tokens to AG Grid params
- **`floqast.app/completeness`** is the production reference — always inspect its DOM before writing any fix or new pattern
- **Figma is the source of truth** for all visual and layout decisions — always check Figma before writing any fix

### Build Phases
- **Phase 1** — Structure and layout with static mock data. No real data wiring. ✅ Complete
- **Phase 2** — Cell renderers: badges, avatars, toggles, links, icons
- **Phase 3** — Real data wiring + filters + sorting

### Mock Data
- **Entities:** Beckys Donuts (6 rows), Danielle Donuts (4 rows)
- **People:** Edson Rodri, Gus Milnor, Alex Noodie + LOTR cast (Dominic Monaghan, Ian McKellen, Miranda Otto, Liv Tyler, Viggo Mortensen, Cate Blanchett, Sean Bean, Orlando Bloom, Sean Astin, Elijah Wood, Hugo Weaving)
- **Source file:** `recs_mock_data.xlsx`

### Process Rules
1. **Always read the actual file before making any change** — never guess at current state
2. **Check `floqast.app/completeness` DOM before writing any fix** — inspect how FloQast implements the pattern in production first
3. **Check the browser DOM before AND after every change** — verify the fix landed correctly
4. **Fix one thing at a time** — no bundling multiple fixes into one prompt
5. **Figma is always checked first** for any visual decision before writing code

### Deferred / Out of Scope
- **⋮ column menu** — requires AG Grid Enterprise (`agMultiColumnFilter`). Skip until eng team confirms licensing.
- **Multi-currency column groups** — not in scope for Phase 2. Requires design work first. Three layers: Transactional (local, e.g. JPY), Functional (operating, e.g. MXN), Reporting (parent, e.g. USD). Owned by Edson. Planned Q2/May. When implemented, use AG Grid native column grouping (`children` in column defs) — style per AG Grid Quartz theme and Completeness patterns.
