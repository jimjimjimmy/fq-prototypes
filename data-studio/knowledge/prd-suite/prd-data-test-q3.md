...
| **Related sub-PRDs** | [1 of 4: Model Creation & Source Configuration](https://floqast.atlassian.net/wiki/spaces/Data/pages/4450189505) · [2a of 4: Field Mapping (6/30)](https://floqast.atlassian.net/wiki/spaces/Data/pages/4449927443) · [2b of 4: Mapping Expressions v2](https://floqast.atlassian.net/wiki/spaces/Data/pages/4594892801) · [4 of 4: Versioning & Lifecycle](https://floqast.atlassian.net/wiki/spaces/Data/pages/4443013309) |
| **Supersedes** | [3 of 4: Testing & Publishing (original Q2 PRD)](https://floqast.atlassian.net/wiki/spaces/Data/pages/4449468593) |

> **Note:** This capability was originally scoped in Model Creation — 3 of 4: Testing & Publishing (DRAFT) but was deferred to Q3. In Q3 planning, engineering noted that implementation approaches may differ by connector type (CDC vs API vs File).
...
An admin has defined mapping expressions for several fields (date format conversions, COALESCE logic, CASE_WHEN normalization). Before publishing, the admin opens the test panel and runs the model against a selected date's live data. The test results show both raw source values and computed mapped outputs side by side. The admin identifies a type coercion failure on one field, uses "Fix this" to navigate to the field mapping row, corrects the expression, saves, and re-runs the test — all without leaving the mapping context.
...
### UC4 — Setup validation before live data flows

An implementation partner is setting up a new customer's GL transaction model, sourced from the customer's ERP via SFTP. The customer hasn't pushed their first real file yet — the connector is configured, the sample file has been uploaded, and the field mappings are defined, but no live data has flowed. The admin uses the sample file path to validate the mapping logic against the uploaded sample before go-live. The sample data path is intentionally surfaced as an explicit option in the as-of date picker, not hidden behind a fallback.

### UC5 — Publishing the model after a clean test
...
- Mapping logic requires explicit save (PRD 2b) — the test reflects the current saved draft state, not a separately saved snapshot
...
- **Two-path data source model:**
  - Sample file path — uses the connector's uploaded sample file; surfaced as an explicit option in the as-of date picker; available even before live data has flowed
...
- Sensitive column error handling (error cards that suppress raw values for sensitive fields) — post-Q3
...
| DT4 | View and Resolve Field Mapping Errors | High |
| DT5 | Publish Model | High |
...
**AC-DT1-02 — Test reflects current saved draft state**

Given I have made changes to field mappings or expressions in the current draft and saved them

When I open or refresh the test panel

Then the test reflects the current saved draft state (not the last published version)
...
**Details:** The as-of date picker surfaces two sections — Sample Data and Live Data — as an intentional, explicit choice. Sample Data uses the connector's uploaded sample file; it is available even before live data has flowed, making it useful for initial setup scenarios (e.g., an implementation partner configuring a new customer's model). Live Data tests against real production data for the selected date. Both paths must be clearly labeled in the UI with enough context for the admin to choose deliberately. The default is the most recent date/source with available data.
...
### DT4 — View and Resolve Field Mapping Errors
...
**AC-DT4-02 — Runtime data errors captured**
...
**AC-DT4-03 — Error detail per field**
...
And the raw failing value may be included where available to aid diagnosis

**AC-DT4-04 — Error count summary in header**
...
**AC-DT4-05 — "Fix this" navigates to affected field mapping row**
...
**AC-DT5-01 — Publish button accessible from model header**
...
**AC-DT5-02 — Publish blocked if mandatory fields are unmapped**
...
**AC-DT5-03 — Effective Date prompt before publish completes**
...
**AC-DT5-04 — Confirm Publish is deliberate**
...
**AC-DT5-05 — Draft transitions to Active on successful publish**
...
**AC-DT5-06 — Failed publish returns model to Draft**
...
| Step 22 | Resolve errors via "Fix this" → field mapping row → save → re-run |
...
- **Sensitive column error handling** — When a mapping error occurs on a field designated as sensitive, suppress the raw failing value from error detail cards (show field name, error reason, and row reference only). Post-Q3; requires the sensitive field designation to be propagated from the field mapping layer into the test surface.
...
| OQ-NEW-1 | Does the field-level mapping expression detail UI need to be redesigned before Q3 ships, or does PRD 2b land first? Sequencing and dependency needed. | Open — PM / Engineering |
| OQ-NEW-2 | If new data syncs for a connector while the admin has the test panel open, does the as-of date picker update? Does an in-progress or completed test reflect the pre-sync or post-sync state? Proceed with stale data silently, or surface a "new data available — re-run?" notice? | Open — Engineering / PM |
...
- [2b of 4: Mapping Expressions v2](https://floqast.atlassian.net/wiki/spaces/Data/pages/4594892801)
