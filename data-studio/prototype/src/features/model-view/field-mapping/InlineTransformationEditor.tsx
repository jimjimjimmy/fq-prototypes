/**
 * Note: this editor is currently unreachable — FieldMappingView no longer
 * sets `editMode === 'inline'` after the AI removal. The file is kept on
 * disk in case the inline-edit mode is revived (would need a non-AI
 * entry point). The AI sparkle button + `useAiMode()` hook were stripped
 * per the "no AI surfaces" decision. To restore: re-add `useAiMode` and
 * the sparkle button matching the original file in
 * `legacy-prototype-work/prototype-legacy/src/components/field-mapping/InlineTransformationEditor.tsx`.
 */
