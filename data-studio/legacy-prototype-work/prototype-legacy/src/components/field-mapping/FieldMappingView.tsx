...
import DirectedTransformationDropdown from './ai/DirectedTransformationDropdown'
import ConversationalAiPanel from './ai/ConversationalAiPanel'
import RunTestModal from './RunTestModal'
import PublishDialog from './PublishDialog'

type EditMode = 'none' | 'inline' | 'floating' | 'dropdown' | 'chat-panel'
...
export default function FieldMappingView() {
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (editMode === 'dropdown' && dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
        setEditMode('none')
  ...
  const handleEditTransformation = useCallback((mapping: FieldMapping) => {
    ...
    if (aiMode === 'directed') {
      setDropdownPos({ top: 200, left: 600 })
      setEditMode('dropdown')
    } else {
  ...
  function handleDropdownInsert(expression: string) {
    if (!activeMapping) return
    setMappings(prev =>
      prev.map(m =>
        m.id === activeMapping.id
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Directed mode dropdown */}
      {editMode === 'dropdown' && activeMapping && dropdownPos && (
        <div
          ref={dropdownRef}
          className="fixed z-50 w-[400px]"
          style={{ top: dropdownPos.top, left: dropdownPos.left }}
        >
          <DirectedTransformationDropdown
            sourceFieldNames={activeMapping.sourceFields.map(f => f.name)}
            onInsert={handleDropdownInsert}
            onClose={handleClose}
          />
        </div>
      )}

      {/* Conversational panel (standalone, below-table) */}
      {editMode === 'chat-panel' && activeMapping && (
        <div className="fixed bottom-20 right-4 z-50 w-96">
          <ConversationalAiPanel
            mapping={activeMapping}
            onInsert={(expr) => {
              handleSaveTransformation(expr)
            }}
            onClose={handleClose}
          />
        </div>
