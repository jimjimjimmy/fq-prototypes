...
| Related sub-PRDs | Schema Definition · Accounting Config · Error Patterns · Logging & Audit |
...
| --- | --- | --- |
| Step 5 UI — Entity Configuration (pattern selection + reconciliation tables) | Natasha Clark | TBD |
...
* Then a table is displayed with one row per detected entity prefix, columns representing the shared fields, and cells showing the specific column that maps to each field for each entity
...
* When the reconciliation panel renders
* Then a table is displayed with one row per detected column block, columns representing the repeating fields within each block, and cells showing the specific column mapped to each field for each block
...
* Schema Definition — Step 3; provides the column list used in entity pattern detection
* Accounting Config — Step 4; entity config is a separate step and must not be conflated
* Error Patterns & Handling — entity mapping failures need to be added as new error cases
* Logging & Audit — entity assignment events should be logged per file load
