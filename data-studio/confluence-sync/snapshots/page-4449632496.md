> not done in Q2, superseded by [https://floqast.atlassian.net/wiki/spaces/Data/pages/4619927662](https://floqast.atlassian.net/wiki/spaces/Data/pages/4619927662)  
...
**Details:** When all pre-requisites are met (Connection exists, Model has had at least one successful pipeline run), the Data Preview tab loads the most recent processed data from the Active version in an AG-Grid.
...
* **And** the most recent Active version is selected by default in the version selector. In scenarios where there is no "Active" version, we should show the most recent version with a successful pipeline run.
...
* **And** custom columns reflect the customer's selection (rather than whatever the names may be in the database.)
...
## ▶️ User Flow Reference
 ...
 1. User navigates to Data Studio

 2. User selects Models from the left navigation

 3. User selects a specific Model from the model list

 4. User clicks the "Data Preview" tab in the Model detail view

 5. System evaluates pre-requisites (Connection, Active version, successful pipeline run)

 6. If any pre-requisite is not met → display the appropriate contextual empty state (see DP6-LC)

 7. If all pre-requisites are met → load the most recent Active version data into the AG-Grid

 8. AG-Grid displays processed target data with column headers, sort controls, and filter icons

 9. User interacts with the grid (sort by column, apply filters, resize/reorder columns, paginate)

10. User optionally opens the version selector and selects an Active or Archived version

11. If version changed → system reloads the grid with the selected version's data and updates column structure accordingly

12. User completes review and navigates away
...
* Should the version selector be displayed as a dropdown above the grid, or inline with the tab header? Is version selectable from the Data Preview tab, or is there some other way to navigate to here?
...
* What should happen when a new pipeline run completes while the user has the Data Preview tab open — auto-refresh, or a "new data available" banner with a manual refresh action?
...
* AI-assisted data exploration (DP7-LC): Natural language query bar that translates user questions into filter criteria — deferred to V2. Feasibility and UX approach to be confirmed before design begins.
...
| --- | --- | --- | --- | --- |
| OQ-1 | What is the data retention policy for data processed by Archived versions? Should we define a retention window (e.g., retain for 2 years) or retain indefinitely? This must be answered before the audit use case can be committed to. | Engineering / Head of Data | Open |  |
...
| OQ-3 | Should the AI-powered data exploration feature (DP7-LC) be included in V1 or deferred to V2? The core grid must ship first regardless. | PM | Open |  |
...
| OQ-5 | Are there any PII or data sensitivity concerns with displaying raw pipeline output in the Data Preview UI? Are field-level masking or role-based access controls needed beyond existing Model permissions? | Engineering / Security | Open |  |
| OQ-6 | Should column display names in the grid use the raw target field name from the data, or a human-readable label sourced from the Model field mapping configuration? What is the fallback if no label exists? | PM / Engineering | Open |  |
| OQ-7 | What should happen when a new pipeline run completes while the user has the Data Preview tab open — should the data auto-refresh, or should we show a "new data available — click to refresh" indicator? | PM / UX / Engineering | Open |  |
...
| --- | --- | --- | --- |
| G1 | The Model versioning system must expose version state (Active / Archived / Draft) to the Data Preview API layer. This dependency needs confirmation from Engineering before V1 scope can be finalized. | Blocks V1 implementation | Confirm with Engineering pre-V1 |
| G2 | No data retention policy currently exists for data processed by Archived versions. Engineering and Head of Data must define and document this policy before the audit use case (DP5-LC) can be committed to in V1. | Blocks audit use case commitment | Head of Data / Engineering to define policy pre-V1 |
