...
* **Given** I navigate to Data Studio
* **Then** the Catalog tab is selected by default _(see A1)_
...
* **Then** they are redirected to the Catalog tab _(see A5)_
...
* **Given** I navigate to Data Studio → Catalog
* **Then** I see the AG Grid model table as defined in the [Catalog: Model Search & Filter PRD](https://floqast.atlassian.net/wiki/spaces/Data/pages/4476731473)
...
**Details:** The Overview tab is the default view when a Model detail is opened. It shows at-a-glance health and status information, while removing connector-level configuration (see CL-6). Linked Files/Tables are accessible in the **Source Datasets tab** (see CL-4).
...
* **Then** I see Total Records, Data Freshness, Last Sync Status, and current Version _(see A4)_

**AC-CL-5-02 — Product Features section shows enabled capabilities**

* **Given** I am on the Overview tab
* **Then** I see the Product Features section showing which FQ product capabilities are enabled for this model (Reconciliations, AI Matching, AI Variance, JEM) _(see A2)_

**AC-CL-5-03 — Connector credentials and schedule are not shown**

* **Given** I am on the Overview tab
* **Then** I do not see connector-level fields: Sync Frequency, Next Sync timestamp, or connector credentials _(see CL-6)_

---

### CL-6 — Remove Connector Configuration from Model Detail

**User Story:** As a FloQast Admin, I want connector settings to live on the Connectors tab so that I know where to go when I need to manage a connection — and model views stay focused on model configuration.
...
* **Then** the connector name is shown as a label identifying which connector the file comes from _(see OQ-3)_
* **And** clicking the connector name does not navigate to connector settings (no inline editing)

---

### CL-7 — Define Full 6-Tab Model Detail Structure

**User Story:** As a FloQast Admin, I want the Model detail to have a complete, clearly organized tab structure — Overview, Source Datasets, Field Mappings, Data Preview, Versions, and Logs — so that all model management tasks are accessible from one place.
